...
Eres Yani, la asistente virtual y asesora experta de repostería de 'Tortas Yani'. Tu objetivo es guiar a los clientes de forma proactiva, inteligente y fluida para que elijan la torta ideal y concretar sus pedidos agregándolos al carrito.
  ...
  Future<String> sendMessage(String userMessage) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data["success"] == true && data["reply"] != null && (data["reply"] as String).trim().length > 10) {
      ...
      print("=== EXCEPTION / TIMEOUT CHAT API === $e");
    }

    // Fallback to Smart Mini-Agent Engine
    final fallbackReply = _generateSmartFallbackReply(userMessage);
    final regex = RegExp(r'\[ADD_CART:[^\]]+\]');
    final cleanReplyForMemory = fallbackReply.replaceAll(regex, '').trim();

    _messages.add(ChatMessage(role: "assistant", content: cleanReplyForMemory));
    return fallbackReply;
  }

  String _generateSmartFallbackReply(String userMessage) {
    final text = userMessage.toLowerCase().trim();
    final historyText = _messages.map((m) => m.content.toLowerCase()).join(' ');

    final isConfirmation = RegExp(r'^(sí|si|claro|dale|de acuerdo|confirmo|agrega|agregalo|listo|ok|perfecto|comprar|por favor)', caseSensitive: false).hasMatch(text) ||
                           text.contains('agrega al carrito') || text.contains('sí agregalo') || text.contains('si por favor');

    if (isConfirmation) {
      String cakeName = 'Torta de Chocolate';
      String sizeChar = 'M';
      String sizeText = 'mediana';
      double price = 85.0;
      String filling = 'Manjar blanco';
      String color = 'Blanco perla';
      String message = 'Sin mensaje';

      if (historyText.contains('boda') || historyText.contains('matrimoni') || text.contains('boda')) {
        cakeName = 'Torta Matrimonial';
        price = 250.0;
        color = 'Blanco perla';
        message = '¡Feliz Matrimonio!';
      } else if (historyText.contains('quince') || text.contains('quince')) {
        cakeName = 'Torta de Quinceañera';
        price = 200.0;
        color = 'Rosa pastel';
        message = 'Mis 15 Años';
      } else if (historyText.contains('zanahoria') || text.contains('zanahoria')) {
        cakeName = 'Torta de Zanahoria';
        price = 65.0;
      } else if (historyText.contains('maracuyá') || historyText.contains('cheesecake') || text.contains('maracuyá')) {
        cakeName = 'Cheesecake de Maracuyá';
        price = 80.0;
        filling = 'Maracuyá';
        color = 'Sin color';
      } else if (historyText.contains('limón') || historyText.contains('pie') || text.contains('limón')) {
        cakeName = 'Pie de Limón';
        price = 55.0;
        filling = 'Limón';
        color = 'Sin color';
      } else if (historyText.contains('red velvet') || text.contains('red velvet')) {
        cakeName = 'Red Velvet';
        price = 90.0;
        filling = 'Queso crema';
      } else if (historyText.contains('tres leches') || text.contains('tres leches')) {
        cakeName = 'Tres Leches';
        price = 70.0;
        filling = 'Manjar blanco';
      } else if (historyText.contains('frutos del bosque') || text.contains('frutos')) {
        cakeName = 'Torta de Frutos del Bosque';
        price = 95.0;
        filling = 'Crema y frutos';
      } else if (historyText.contains('vainilla') || text.contains('vainilla')) {
        cakeName = 'Torta de Vainilla';
        price = 60.0;
      }

      if (text.contains('grande') || text.contains('l') || historyText.contains('grande')) {
        sizeChar = 'L';
        sizeText = 'grande';
        if (cakeName == 'Torta de Chocolate') price = 115.0;
        if (cakeName == 'Torta Matrimonial') price = 338.0;
      } else if (text.contains('pequeña') || text.contains('pequeño') || text.contains('s')) {
        sizeChar = 'S';
        sizeText = 'pequeña';
        if (cakeName == 'Torta de Chocolate') price = 64.0;
      }

      final magicTag = '[ADD_CART:$cakeName|$sizeChar|1|$filling|$color|$message|${price.toStringAsFixed(1)}]';
      return '¡Excelente! He agregado tu $cakeName $sizeText al carrito por S/ ${price.toStringAsFixed(0)}.\n\nPor favor selecciona el ícono del carrito en la esquina superior derecha para elegir tu fecha, hora de entrega y concretar el pago.\n\n$magicTag';
    }

    if (text.contains('boda') || text.contains('matrimoni') || text.contains('casamiento')) {
    ...
    if (text.contains('quince') || text.contains('15 años')) {
    ...
    if (text.contains('chocolate') || text.contains('choc')) {
    ...
    if (text.contains('maracuyá') || text.contains('cheesecake')) {
    ...
    if (text.contains('limón') || text.contains('pie')) {
    ...
    if (text.contains('persona') || text.contains('invitado') || text.contains('tamaño') || text.contains('porcion')) {
